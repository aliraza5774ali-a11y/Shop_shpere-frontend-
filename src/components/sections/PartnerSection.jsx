  ...
  return (
    <section ref={sectionRef} className={`w-full ${bg} overflow-hidden`}>
      ...
      <motion.div
        ...
        className="flex items-center justify-between px-28 py-6 border-b border-black/6"
      >

        {/* Left — avatars + rating + trust */}
        <div className="flex items-center gap-5">
          ...
          <div className="flex items-center gap-1.5">
            <div className="flex items-center gap-0.5">
              {[...Array(5)].map((_, i) => (
                <Star key={i} size={13} fill={ratingColor} stroke={ratingColor} strokeWidth={1.5} />
              ))}
            </div>
          ...
          {/* Trust text */}
          <span className="text-xs font-medium text-black/60 tracking-wide">{trustText}</span>

        </div>

        {/* Right — logo ticker contained */}
        <div
        ...
        >
          <motion.ul
            ...
            transition={{ repeat: Infinity, repeatType: "loop", duration: tickerSpeed, ease: "linear" }}
          >
            {tickerLogos.map((logo, i) => (
              <li key={i} className="flex-shrink-0 flex items-center justify-center" style={{ width: logo.w, height: 36 }}>
                <img
                  src={logo.src}
                  alt=""
